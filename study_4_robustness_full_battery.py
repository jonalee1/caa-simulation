...
Battery (eight tests, every cell x two modes):
    ...
    7. Ordinal response scales  continuous reference vs. K in {7, 5, 4}
                                (per-item threshold discretization, J = 5),
                                plus the PHQ-9 configuration J = 9, K = 4
...
def apply_threshold_discretization(dataset: Dict, n_categories: int,
                                   probs: Optional[np.ndarray] = None) -> Dict:
    """
    Threshold discretization of the latent continuous response: per-item
    thresholds are placed at prespecified cumulative proportions of each
    item's empirical distribution, and observed categories 0..K-1 are
    assigned by threshold crossing (graded-response-style generation).

    Default probs are equal-frequency (each category ~1/K of responses),
    which yields plausible, non-degenerate category use. Per-item monotone
    thresholds are NOT a per-person affine transform, so discretization
    legitimately perturbs the within-person profile; the rounding is the
    stressor, not a tail-sensitive rescaling.
    """
    ds = dict(dataset)
    x = np.asarray(ds["responses"], dtype=np.float64)
    if probs is None:
        probs = np.full(n_categories, 1.0 / n_categories)
    probs = np.asarray(probs, dtype=np.float64)
    if len(probs) != n_categories or not np.isclose(probs.sum(), 1.0):
        raise ValueError("probs must have length n_categories and sum to 1")
    cum = np.cumsum(probs)[:-1]                      # K-1 interior cut points
    x_cat = np.zeros_like(x)
    for j in range(x.shape[1]):
        thresholds = np.quantile(x[:, j], cum)
        x_cat[:, j] = np.searchsorted(thresholds, x[:, j], side="right")
    ds["responses"] = x_cat.astype(np.float32)
    return ds
...
def test_ordinal(n_replicates: int, verbose: bool = True) -> List[Dict]:
    if verbose:
        print("\nTest 7: Ordinal response scales (per-item threshold "
              "discretization; includes PHQ-9 configuration J = 9, K = 4)")
        print("-" * 78)
    results = []
    results.extend(run_cell_both_modes(
        lambda s: generate_identifiable(seed=s),
        n_replicates, test="ordinal", cell="continuous",
        label="continuous (reference)", verbose=verbose))
    for cats in (7, 5, 4):
        results.extend(run_cell_both_modes(
            lambda s, c=cats: apply_threshold_discretization(
                generate_identifiable(seed=s), n_categories=c),
            n_replicates, test="ordinal", cell=f"J5_K{cats}",
            label=f"J = 5, K = {cats}", verbose=verbose))
    results.extend(run_cell_both_modes(
        lambda s: apply_threshold_discretization(
            generate_identifiable(n_items=9, seed=s), n_categories=4),
        n_replicates, test="ordinal", cell="J9_K4",
        label="J = 9, K = 4 (PHQ-9 configuration)", verbose=verbose))
    return results
